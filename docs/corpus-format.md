...
A directory is a Hintforge corpus if and only if it contains the three always-present published directories and nine universal files listed below, plus a manifest at one of two sanctioned locations: `nav/architecture.md` when spatial navigation is modeled, or root `architecture_manifest.md` when `nav/` is intentionally skipped. The reader can assume the always-present core exists and resolves the manifest from either known path. (For cheap corpus *detection* the reader checks only a subset of always-present markers rather than all nine -- see the reader skill's session-start step 1 -- but the full set below is what a complete corpus carries. This document is the authority for the universal-file list; the reader's detection subset points back here.)
...
**Published core directories** (`items/`, `sections/`, and `_overflow/` are always present; `nav/` is conditional):

| Directory | Purpose |
|---|---|
| `nav/` | **Conditional.** Present when spatial navigation is worth modeling; contains per-zone routing files plus `architecture.md` (zone graph, optional content registry, support topology, locks-and-keys, vector list). Omitted for `narrative-no-nav` games and other sanctioned nav-skip cases where a rich in-game map makes corpus routing unnecessary. When omitted, the manifest and vector list live at root `architecture_manifest.md`. When present, this is the source of truth for routing (Rule 1), lookahead (Rule 2), backtrack (Rule 3), reachability (Rule 4), and locks (Rule 5). See §3 and [`../setup_wizard.md`](../setup_wizard.md) Step 7. |
...
A reader scanning a directory for "is this a corpus?" checks the three always-present directories, the universal files, and a manifest at either sanctioned location. The presence of builder-only or platform/runtime paths does not affect the answer.
...
**File counts vary by more than an order of magnitude across vectors and games.** When present, `nav/` ranges from a couple of files to several dozen; `puzzles/` ranges from zero to a dozen-plus. The reader must handle a sanctioned nav-skip corpus, empty-or-near-empty always-present core folders, and variable populations within extensions.
...
**Primary: vector list in the manifest.** Resolve the manifest by known path: `nav/architecture.md` when `nav/` exists, otherwise root `architecture_manifest.md`. The manifest carries a `Vector extensions` section listing the extension folders this corpus uses and a one-line semantic for each. Format:
...
The reader reads this section from the resolved manifest at session start, registers each extension and its semantic, and routes topical questions accordingly.

**Fallback: filesystem listing.** If the resolved manifest has no `Vector extensions` section (corpus is mid-construction, or the maintainer hasn't populated the list), the reader lists top-level directories in the corpus, excludes the three always-present core directories (`items/`, `sections/`, `_overflow/`), conditional `nav/` when present, the optional `mechanics/` split directory, and known platform/runtime directories (`ptt/`, `save_state/`, `.claude/`), and treats the remainder as vector extensions with unknown semantics. The reader still routes to them on best-guess name matching but flags the missing manifest section to the maintainer.
...
`corpus-core-version` is an integer stamped in the `## Hintforge manifest` block of the resolved manifest (`nav/architecture.md` or root `architecture_manifest.md`). It increments only when a change to this document (`docs/corpus-format.md`) would break an older reader -- new required fields, removed fields, renamed fields, or any change to the always-present core directory/file set that the reader hard-codes. New vector extensions, new content within existing files, persona iteration, prose edits, documentation corrections that align the contract with already-supported reader behavior, and other non-format-breaking work do not bump the version.

**Version history:**

- **v1** -- initial published format (Phase 3 launch). Universal core documented at launch as four directories and nine files; claim fields `claim`, `source`, `contributor`, `confidence`, `last-verified`, `enemy-tier`, `puzzle-tier`, `category`; optional `spoiler`, `conflicts-with`, `game-version`, `platform`. The current contract's nav-skip path documents behavior later sanctioned by the builder and already tolerated by the reader; it does not introduce a new incompatible core version.
...
When `nav/` is present, `nav/architecture.md` is the corpus's spatial structural spine. It carries five primitives the reader uses for navigation-aware behavior:
...
It also carries the **manifest and vector list** described in §3. In a sanctioned nav-skip corpus, those non-spatial sections move to root `architecture_manifest.md`; the five spatial primitives above are intentionally absent rather than missing.
