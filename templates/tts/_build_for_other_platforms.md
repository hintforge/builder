...
2. **Read every file under `windows/`** -- `tts_hook.ps1`, `tts_control.ps1`, `tts_voices.txt`, `commands/tts.md`, `settings.json`. Understand each file's job before translating.
...
| `windows/tts_control.ps1` | Control script for `/tts`. Takes `start|stop|toggle|status`; touches/removes a `tts_disabled.flag` file and silences in-progress speech. | **`tts_control.sh`** -- same verbs, `touch` / `rm` for the flag, kill the player PID. | **`tts_control.sh`** -- same as Mac. |
...
- **`chmod +x` on shell scripts.** Newly-written `.sh` files default to non-executable. The agent must explicitly `chmod +x tts_hook.sh tts_control.sh` (or the hook will silently fail to fire -- Claude Code's hook runner just logs `permission denied` and moves on).
