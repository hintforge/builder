...
> 🧠 **Run on a mid-tier, cost-efficient model (Sonnet-class or equivalent) at reasoning effort `high`.** Same discipline as ingestion: run at reasoning effort **`high`** -- the level named `high` (Anthropic `effort: high`, Sonnet's default in Claude Code; Codex/GPT-5 `model_reasoning_effort: high`), NOT your runtime's maximum where it exposes higher rungs (`xhigh` / `max` / adaptive). Capable enough, not overkill: keep the model mid-tier (not a flagship) and the effort at `high`, not maxed -- a bigger model or a higher rung over-reasons and costs more for no gain. Reconciling conflicts and writing edges are judgment calls -- give them the reasoning headroom.
...
3c. **Corpus cross-reference lint (mandatory).** Two classes of reference are asserted outside `dependencies.md` and so are invisible to the per-edge audit, which reads edge *content* in that one file. Both resolve to the same defect -- the corpus points at something that does not exist -- and both survive repeated passes silently.
   ...
   - **Dangling file references.** A prose pointer of the shape ``see corpus `<path>.md` ``, ``full <topic> in `<path>.md` ``, or a bare inline `` `<subfolder>/<file>.md` `` can name a file that was never written. Grep the corpus for the cross-reference shapes it actually uses and report the targets with no file on disk. **Resolve each hit against the referencing file's own directory first, then against the corpus root** -- a pointer written inside a topic folder normally names its sibling by bare filename, and resolving from the root alone reports every one of those as dangling. Keep the pattern set deliberately narrow: matching every `.md`-shaped string (including link examples and fenced code) produces more false positives than the check is worth. Grow the set as new shapes appear rather than widening it to a catch-all. Scope the scan to corpus content -- exclude the research inbox and any archive folder, whose files are raw inputs carrying references written by outside sources, not corpus claims the guide is answerable for.
   ...
   ```
   grep -rnoE --exclude-dir=research_inbox --exclude-dir=archive '(see corpus|[Ff]ull [a-z ]+ in) `[a-z0-9_/-]+\.md`' . | while IFS= read -r hit; do src=${hit%%:*}; t=$(printf '%s' "$hit" | grep -oE '`[a-z0-9_/-]+\.md`' | tr -d '`'); [ -f "$(dirname "$src")/$t" ] || [ -f "$t" ] || echo "$hit -> MISSING $t"; done
   grep -rhoE '`\([a-z0-9_-]+, *[a-z0-9_-]+\)`' nav/ | tr -d '`() ' | sort -u | while IFS=, read -r from to; do if grep -qw "$from" nav/architecture.md && ! grep -qw "$to" nav/architecture.md; then echo "endpoint '$to' declared from '$from' but absent from architecture"; fi; done
   ```

   Two anchors are what make the endpoint check usable, and both were established by running it across every available corpus before it shipped. **Match only a declared edge, not any parenthetical:** a node file writes its edges as a marked-up `(from, to)` pair under its outgoing-edges heading, while ordinary prose is full of unmarked pairs -- a sector described as `(lift, southeast)`, a pickup tagged `(optional, item-pickup)`. Matching bare parentheses reported a false positive in three of four corpora; requiring the code-span markup left exactly one finding, and it was the real broken edge. **Then require the origin to be known:** report a target only when the id it was declared *from* is one the architecture already recognizes, and match ids on word boundaries so a short id does not match as a substring of an unrelated word.
