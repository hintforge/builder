...
Plain-language definitions for the project-specific vocabulary. This is a glossary, not a specification -- for the on-disk format contract, see [`docs/corpus-format.md`](docs/corpus-format.md); for the runtime behavior, see the [`hintforge-reader`](https://github.com/dtiger1889-ops/hintforge-reader) skill.
...
- **Universal core.** The four directories (`nav/`, `items/`, `sections/`, `_overflow/`) and nine files (`CHECKPOINT.md`, `CLAUDE.md`, `controls.md`, `settings.md`, `limitations.md`, `warning_tiers.md`, `mechanics.md`, `persona.md`, `dependencies.md`) that every corpus has, regardless of game type. The reader can assume these exist (some -- `dependencies.md`, `_overflow/` -- may ship at status `scaffold`).
