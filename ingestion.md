...
> 🧠 **Run ingestion on a Sonnet-class (or equivalent) model at high reasoning effort.** Ingestion makes real judgment calls -- spoiler-tier classification, confidence calibration, routing facts to the right file by vector, reconciling against existing corpus content -- so give it the high end of your assistant's effort ladder (Anthropic `effort: high`; Codex/GPT-5 `model_reasoning_effort: high`; the top graduated level otherwise). Confirm your model and effort setting before triggering.
